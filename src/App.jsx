...
const selectRecipeImage = (title = '', ingredients = []) => {
  ...
  if (text.includes('pollo') || text.includes('pavo') || text.includes('ave')) return RECIPE_IMAGES_DB.pollo[Math.floor(Math.random() * RECIPE_IMAGES_DB.pollo.length)];
  if (text.includes('carne') || text.includes('ternera') || text.includes('bistec') || text.includes('hamburguesa')) return RECIPE_IMAGES_DB.carne[Math.floor(Math.random() * RECIPE_IMAGES_DB.carne.length)];
  if (text.includes('salmon') || text.includes('salmón') || text.includes('atun') || text.includes('atún') || text.includes('pescado') || text.includes('merluza')) return RECIPE_IMAGES_DB.pescado[Math.floor(Math.random() * RECIPE_IMAGES_DB.pescado.length)];
  if (text.includes('ensalada') || text.includes('bowl') || text.includes('verde')) return RECIPE_IMAGES_DB.ensalada[Math.floor(Math.random() * RECIPE_IMAGES_DB.ensalada.length)];
  if (text.includes('pasta') || text.includes('macarrones') || text.includes('espagueti')) return RECIPE_IMAGES_DB.pasta[Math.floor(Math.random() * RECIPE_IMAGES_DB.pasta.length)];
...
// ==========================================
// FUNCIÓN BLINDADA PARA LEER DATOS (Evita que la app colapse)
// ==========================================
const getSafeStorage = (key, fallback) => {
  try {
    ...
    console.warn(`Dato corrupto en localStorage para ${key}. Restaurando por defecto.`);
    return fallback;
  }
};
...
export default function App() {
  ...
  const [userProfile, setUserProfile] = useState(() => getSafeStorage('active_user_profile', null));
  ...
  // ==========================================
  const DEV_EMAIL = 'tu@email.com'; // <--- CAMBIA ESTO POR TU CORREO REAL
  ...
  const [startDate] = useState(() => {
    ...
    if (saved && !isNaN(new Date(saved).getTime())) return new Date(saved);
    const now = new Date();
  ...
  const [dailyCredits, setDailyCredits] = useState(() => {
    ...
    return saved !== null && !isNaN(Number(saved)) ? Number(saved) : 10;
  });

  const [isPro, setIsPro] = useState(() => isDeveloper || localStorage.getItem(`${userKey}_is_pro`) === 'true');
  const [showPaywallModal, setShowPaywallModal] = useState(false);

  // Estados cargados con el blindaje "getSafeStorage"
  const [bodyLogs, setBodyLogs] = useState(() => getSafeStorage(`${userKey}_body_logs`, INITIAL_BODY_MEASUREMENTS));
  const [dailyNutritionLogs, setDailyNutritionLogs] = useState(() => getSafeStorage(`${userKey}_nutrition_logs`, {}));
  const [logs, setLogs] = useState(() => getSafeStorage(`${userKey}_workout_logs`, {}));
  const [userXP, setUserXP] = useState(() => {
    const saved = localStorage.getItem(`${userKey}_xp`);
    return saved !== null && !isNaN(Number(saved)) ? Number(saved) : 0;
  });
  const [userPhotos, setUserPhotos] = useState(() => getSafeStorage(`${userKey}_photos`, {}));
  ...
  const analyticsData = EXERCISE_ANALYTICS[selectedAnalyticsEx] || EXERCISE_ANALYTICS['bench'];
  ...
  return (
    <div style={rootStyle} className={`min-h-screen ${theme.bg} ${theme.text} flex flex-col justify-between select-none relative transition-colors duration-500`}>
      ...
      <header className={`sticky top-0 z-40 ${theme.navBg} backdrop-blur-xl border-b ${theme.border} px-6 py-4 flex justify-between items-center transition-colors duration-500`}>
        <div>
          <span className={`text-[9px] font-bold tracking-[0.2em] ${theme.muted} uppercase block mb-1`}>ATLETA: {userProfile?.name || 'Invitado'}</span>
          ...
          <div className="space-y-8 animate-fadeIn">
            <div className="space-y-4">
              <div className={`flex justify-between items-center text-[10px] ${theme.muted} font-bold uppercase tracking-widest`}>
                ...
                <button onClick={() => setSelectedWorldModal(currentWorldObj)} className={`${theme.text} flex items-center gap-1 hover:opacity-70 transition-opacity`}>Fase {currentWorldObj?.id || 1} <span>→</span></button>
              </div>
              <div className="w-full overflow-hidden">
                <div className="flex overflow-x-auto gap-3 pb-4 snap-x scrollbar-none px-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  {Array.from({ length: 60 }, (_, i) => i + 1).map((dayNum) => (
                    <button key={dayNum} onClick={() => setSelectedDay(dayNum)} className={`flex-shrink-0 snap-center w-16 h-20 rounded-[1.5rem] flex flex-col items-center justify-center transition-all border ${selectedDay === dayNum ? `${theme.primary} border-transparent scale-105 shadow-lg` : restDays[dayNum] ? `${theme.secondary} ${theme.border} opacity-80` : `${theme.card} ${theme.border} ${theme.muted} hover:opacity-80`}`}>
              ...
              <div className="flex items-center gap-8">
                <div className="relative w-32 h-32 flex items-center justify-center flex-shrink-0">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path className={`${isDark ? "text-white" : "text-black"} transition-all duration-1000`} strokeDasharray={`${calPercentage || 0}, 100`} strokeWidth="2" strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
                ...
                <div className={`border ${theme.border} rounded-[2rem] overflow-hidden space-y-6 pb-6 animate-fadeIn ${theme.secondary}`}>
                  ...
                  <div className="px-6 space-y-6">
                    <div className={`grid grid-cols-3 gap-2 text-center p-3 ${theme.card} rounded-2xl border ${theme.border}`}>
                      <div><span className={`text-[9px] ${theme.muted} font-bold uppercase block`}>Proteína</span><span className="text-sm font-black text-green-500">{personalizedRecipe.prot}g</span></div>
                      <div><span className={`text-[9px] ${theme.muted} font-bold uppercase block`}>Carbs</span><span className="text-sm font-black">{personalizedRecipe.carbs}g</span></div>
                      <div><span className={`text-[9px] ${theme.muted} font-bold uppercase block`}>Grasas</span><span className="text-sm font-black">{personalizedRecipe.fat}g</span></div>
                    </div>
                    
                    <div>
                      <h4 className={`text-[10px] font-bold ${theme.muted} uppercase tracking-widest mb-3 flex items-center gap-2`}><span>🥘</span> Ingredientes Requeridos</h4>
                      <div className="grid grid-cols-1 gap-2">
                        {personalizedRecipe.ingredients?.map((ing, i) => (
                          <div key={i} className={`text-xs font-medium p-3 ${theme.card} rounded-xl border ${theme.border} flex items-center gap-3`}><span className="w-2 h-2 rounded-full bg-emerald-500"></span>{ing}</div>
                        ))}
                      </div>
                    </div>
                    
                    <div>
                      <h4 className={`text-[10px] font-bold ${theme.muted} uppercase tracking-widest mb-3 flex items-center gap-2`}><span>👨‍🍳</span> Paso a Paso</h4>
                      <div className="space-y-3">
                    ...
                    {personalizedRecipe.chefTip && (
                      <div className={`p-4 rounded-2xl border border-amber-500/30 bg-amber-500/5 text-amber-600 dark:text-amber-400 text-xs flex gap-3 items-start`}>
                        ...
                        <div><strong className="block font-bold uppercase text-[9px] tracking-wider mb-1">Tip Nutricional</strong><p className="font-light italic">{personalizedRecipe.chefTip}</p></div>
                      </div>
                    )}
                    ...
                    >Añadir esta comida a mis macros</button>
                  </div>
                </div>
              ) : (
                 <div className="text-center py-8 space-y-3">
                   <span className="text-4xl block">🥗</span>
                   <p className={`text-xs ${theme.muted} font-medium leading-relaxed max-w-xs mx-auto`}>Pulsa "Generar IA" para obtener una receta personalizada con fotografía en HD adaptada a tus macros faltantes de hoy.</p>
                 </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-8 animate-fadeIn pb-6">
            ...
            <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-6 space-y-6 ${theme.shadow} transition-colors duration-500`}>
              <div className={`flex justify-between items-center border-b ${theme.border} pb-3`}>
                <div><span className={`text-[10px] ${theme.muted} font-bold tracking-widest block uppercase mb-1`}>Visual</span><h2 className="text-lg font-black">Día {selectedDay}</h2></div>
                <button onClick={() => setShowManagePhotosModal(true)} className={`text-[10px] ${theme.secondary} px-4 py-2 rounded-full font-bold uppercase tracking-widest`}>Galería</button>
              </div>
              ...
              <div className={`relative w-full aspect-[9/16] ${isDark ? 'bg-black' : 'bg-gray-100'} rounded-[2rem] overflow-hidden border ${theme.border} flex items-center justify-center`}>
                {userPhotos[selectedDay] ? (
                  <><img src={userPhotos[selectedDay]} className="w-full h-full object-cover" /><button onClick={() => { if(window.confirm('¿Borrar foto?')){ const newP={...userPhotos}; delete newP[selectedDay]; setUserPhotos(newP);} }} className="absolute top-4 right-4 bg-black/50 hover:bg-red-600 text-white p-3 rounded-full text-xs backdrop-blur-md">✕</button></>
                ) : (
                  <div className="text-center p-6 space-y-3"><span className="text-4xl block opacity-20">📷</span><p className={`text-xs ${theme.muted} uppercase tracking-widest font-bold`}>Sin foto</p></div>
                )}
              </div>
            ...
            <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 space-y-6 ${theme.shadow} transition-colors duration-500`}>
              ...
              <div className="space-y-5">
                <div><label className={`text-[10px] ${theme.muted} font-bold uppercase tracking-widest block mb-2`}>Peso (kg)</label><input type="number" step="0.1" value={trackerWeight} onChange={(e) => setTrackerWeight(e.target.value)} className={`w-full bg-transparent border-b ${theme.border} py-2 text-2xl font-black outline-none`} /></div>
                <div><label className={`text-[10px] ${theme.muted} font-bold uppercase tracking-widest block mb-2`}>Cintura * (cm)</label><input type="number" required value={trackerWaist} onChange={(e) => setTrackerWaist(e.target.value)} className={`w-full bg-transparent border-b ${theme.border} py-2 text-2xl font-black outline-none placeholder:${theme.muted}`} placeholder="Ej. 82.0" /></div>
                <div><label className={`text-[10px] ${theme.muted} font-bold uppercase tracking-widest block mb-2`}>Pecho (cm)</label><input type="number" value={trackerChest} onChange={(e) => setTrackerChest(e.target.value)} className={`w-full bg-transparent border-b ${theme.border} py-2 text-2xl font-black outline-none placeholder:${theme.muted}`} placeholder="Ej. 104.0" /></div>
              </div>
              <button onClick={handleSavePhysicalTracking} className={`w-full py-4 rounded-full text-[10px] font-black uppercase tracking-widest ${theme.secondary} border ${theme.border} mt-4`}>Guardar Día {selectedDay}</button>
            </div>
          </div>
        )}
      </main>
      ...
      {showCameraModal && (
        <div className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-between p-6 animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-sm flex justify-between items-center py-2"><span className="text-[10px] text-white font-bold uppercase tracking-widest">CÁMARA 9:16</span><button onClick={stopCamera} className="text-gray-500 font-bold text-xl p-2 hover:text-white transition-colors">✕</button></div>
          <div className="relative w-full max-w-sm aspect-[9/16] bg-black rounded-[2rem] overflow-hidden border border-white/20 flex items-center justify-center my-auto shadow-2xl">
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center justify-center p-4 transition-opacity" style={{ opacity: overlayOpacity }}>
              <svg viewBox="0 0 100 200" className="w-full h-full stroke-white/80 fill-none" strokeWidth="1" strokeDasharray="2 1"><circle cx="50" cy="22" r="8" /><path d="M 40 38 L 60 38 M 42 75 L 58 75 M 45 95 L 55 95" /><line x1="20" y1="22" x2="80" y2="22" stroke="rgba(255,255,255,0.3)" strokeWidth="0.5" /><line x1="20" y1="75" x2="80" y2="75" stroke="rgba(255,255,255,0.3)" strokeWidth="0.5" /></svg>
            </div>
          </div>
          <div className="w-full max-w-sm space-y-2 py-4"><div className="flex justify-between text-[10px] font-bold text-gray-500 uppercase tracking-widest"><span>Opacidad Guía:</span><span className="text-white">{Math.round(overlayOpacity * 100)}%</span></div><input type="range" min="0.1" max="1" step="0.1" value={overlayOpacity} onChange={(e) => setOverlayOpacity(parseFloat(e.target.value))} className="w-full accent-white" /></div>
          <button onClick={capturePhoto} className="w-full max-w-sm py-4 bg-white text-black font-black rounded-full text-[10px] uppercase tracking-widest shadow-lg mb-4">DISPARAR FOTO</button>
        </div>
      )}
      ...
      {showCompareModal && (
        <div className="fixed inset-0 bg-black/95 z-50 flex flex-col items-center justify-between p-4 overflow-y-auto animate-fadeIn">
          <div className="w-full max-w-sm flex justify-between items-center py-2"><h3 className="text-[10px] font-bold text-white uppercase tracking-widest">Alineación de Capas</h3><button onClick={() => setShowCompareModal(false)} className="text-gray-500 font-bold text-xl p-2 hover:text-white transition-colors">✕</button></div>
          <div className="w-full max-w-sm space-y-4">
            <div className="flex justify-between items-center bg-[#111] p-4 rounded-[1.5rem] border border-white/10 text-xs"><span className="text-gray-500 font-bold uppercase tracking-wider">Comparar con:</span><select value={compareDay} onChange={(e) => setCompareDay(Number(e.target.value))} className="bg-transparent text-white font-bold outline-none">{Object.keys(userPhotos).map(Number).filter(d => d !== selectedDay).map(d => <option key={d} value={d} className="bg-black">Día {d}</option>)}</select></div>
            <div className="flex bg-[#111] p-1.5 rounded-full border border-white/10 text-[10px] font-bold uppercase tracking-widest"><button onClick={() => setCompareMode('blend')} className={`flex-1 py-3 rounded-full transition-all ${compareMode === 'blend' ? 'bg-white text-black' : 'text-gray-500'}`}>Fusión</button><button onClick={() => setCompareMode('slider')} className={`flex-1 py-3 rounded-full transition-all ${compareMode === 'slider' ? 'bg-white text-black' : 'text-gray-500'}`}>Cortinilla</button></div>
          </div>
          <div className="relative w-full max-w-sm aspect-[9/16] bg-black rounded-[2rem] overflow-hidden border border-white/20 my-4 shadow-2xl flex items-center justify-center">
            {userPhotos[compareDay] && <img src={userPhotos[compareDay]} className="absolute inset-0 w-full h-full object-cover" style={{ transform: `scale(${baseScale}) translate(${baseX}px, ${baseY}px)` }} />}
            {userPhotos[selectedDay] && <div className="absolute inset-0 overflow-hidden" style={{ clipPath: compareMode === 'slider' ? `polygon(0 0, ${sliderPos}% 0, ${sliderPos}% 100%, 0 100%)` : 'none', opacity: compareMode === 'blend' ? blendOpacity : 1 }}><img src={userPhotos[selectedDay]} className="w-full h-full object-cover" style={{ transform: `scale(${topScale}) translate(${topX}px, ${topY}px)` }} /></div>}
          ...
          <div className="w-full max-w-sm bg-[#111] p-6 rounded-[2rem] border border-white/10 space-y-6">
            <div className="flex bg-black/50 p-1.5 rounded-full border border-white/5 justify-between items-center text-[9px] uppercase tracking-widest font-bold"><span className="text-gray-500 px-4">EDITAR:</span><div className="flex gap-1"><button onClick={() => setActiveLayer('base')} className={`px-5 py-2.5 rounded-full transition-all ${activeLayer === 'base' ? 'bg-white text-black' : 'text-gray-500 hover:text-gray-300'}`}>BASE</button><button onClick={() => setActiveLayer('top')} className={`px-5 py-2.5 rounded-full transition-all ${activeLayer === 'top' ? 'bg-white text-black' : 'text-gray-500 hover:text-gray-300'}`}>CAPA</button></div></div>
            {compareMode === 'blend' ? <div><div className="flex justify-between text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-2"><span>Opacidad Sup:</span><span className="text-white">{Math.round(blendOpacity * 100)}%</span></div><input type="range" min="0" max="1" step="0.05" value={blendOpacity} onChange={(e) => setBlendOpacity(parseFloat(e.target.value))} className="w-full accent-white" /></div> : <div><div className="flex justify-between text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-2"><span>Línea:</span><span className="text-white">{sliderPos}%</span></div><input type="range" min="0" max="100" value={sliderPos} onChange={(e) => setSliderPos(Number(e.target.value))} className="w-full accent-white" /></div>}
            <div className="grid grid-cols-3 gap-4">
              <div><label className="text-[9px] font-bold uppercase text-gray-500 block mb-2">Zoom</label><input type="range" min="0.8" max="1.4" step="0.01" value={activeLayer === 'top' ? topScale : baseScale} onChange={(e) => { const v = parseFloat(e.target.value); activeLayer==='top'?setTopScale(v):setBaseScale(v); }} className="w-full accent-white" /></div>
              <div><label className="text-[9px] font-bold uppercase text-gray-500 block mb-2">X (Horiz)</label><input type="range" min="-40" max="40" step="1" value={activeLayer === 'top' ? topX : baseX} onChange={(e) => { const v = parseInt(e.target.value, 10); activeLayer==='top'?setTopX(v):setBaseX(v); }} className="w-full accent-white" /></div>
              <div><label className="text-[9px] font-bold uppercase text-gray-500 block mb-2">Y (Vert)</label><input type="range" min="-40" max="40" step="1" value={activeLayer === 'top' ? topY : baseY} onChange={(e) => { const v = parseInt(e.target.value, 10); activeLayer==='top'?setTopY(v):setBaseY(v); }} className="w-full accent-white" /></div>
            </div>
            <button onClick={() => { setTopScale(1); setTopX(0); setTopY(0); setBaseScale(1); setBaseX(0); setBaseY(0); }} className="w-full py-4 bg-[#222] border border-white/10 text-white font-black rounded-full text-[10px] uppercase tracking-widest">REINICIAR ALINEACIÓN</button>
          </div>
        </div>
      )}
      ...
      {showManagePhotosModal && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 animate-fadeIn">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 max-w-sm w-full space-y-6 max-h-[85vh] flex flex-col shadow-2xl`}>
            <div className={`flex justify-between items-center border-b ${theme.border} pb-4`}><h3 className="text-xl font-black">Galería</h3><button onClick={() => setShowManagePhotosModal(false)} className={`${theme.muted} font-bold text-xl hover:text-white transition-colors`}>✕</button></div>
            <div className="flex-1 overflow-y-auto space-y-4 pr-2">
              {Object.keys(userPhotos).length === 0 ? <p className={`text-center text-sm ${theme.muted} py-10`}>La galería está vacía.</p> : Object.entries(userPhotos).map(([dayNum, photoUrl]) => (
                <div key={dayNum} className={`${theme.secondary} p-3 rounded-[1.5rem] border ${theme.border} flex items-center justify-between gap-4`}><img src={photoUrl} className="w-16 h-24 object-cover rounded-2xl" /><div className="flex-1"><span className="text-sm font-black block">Día {dayNum}</span></div><div className="flex flex-col gap-2"><button onClick={() => { setSelectedDay(Number(dayNum)); setShowManagePhotosModal(false); }} className={`px-5 py-2 ${theme.primary} text-[10px] font-black uppercase tracking-widest rounded-full`}>Ir</button><button onClick={() => { if(window.confirm('¿Borrar?')){ const newP={...userPhotos}; delete newP[dayNum]; setUserPhotos(newP);} }} className="px-5 py-2 bg-transparent text-gray-500 text-[10px] font-black uppercase tracking-widest rounded-full hover:text-red-500 transition-colors">Borrar</button></div></div>
              ))}
            </div>
          </div>
        </div>
      ...
      {selectedWorldModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-fadeIn">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 max-w-sm w-full space-y-6 shadow-2xl`}>
            <div className={`flex justify-between items-start border-b ${theme.border} pb-4`}><div><span className={`text-[10px] ${theme.muted} font-bold tracking-widest block uppercase mb-1`}>Fase Actual</span><h3 className="text-xl font-black leading-tight">{selectedWorldModal.title}</h3></div><button onClick={() => setSelectedWorldModal(null)} className={`${theme.muted} font-bold text-xl`}>✕</button></div>
            <div className="space-y-3"><span className={`text-[10px] ${theme.muted} font-bold block uppercase tracking-widest`}>Ítems</span><div className="grid grid-cols-2 gap-3">{selectedWorldModal.items.map((item, idx) => (<div key={idx} className={`${theme.secondary} p-3 rounded-2xl border ${theme.border} text-[10px] font-bold uppercase tracking-wide flex items-center`}>{item}</div>))}</div></div>
            <div className="space-y-3"><span className={`text-[10px] ${theme.muted} font-bold block uppercase tracking-widest`}>Trofeos</span><div className="grid grid-cols-2 gap-3">{selectedWorldModal.trophies.map((trophyId) => { const trophy = TROPHY_DEFINITIONS[trophyId]; return (<div key={trophyId} className={`p-4 ${theme.secondary} border ${theme.border} rounded-2xl flex flex-col items-center text-center gap-2`}><span className="text-2xl">{trophy.icon}</span><div><h4 className="text-[10px] font-bold uppercase tracking-widest mb-1">{trophy.title}</h4><p className={`text-[9px] ${theme.muted}`}>{trophy.desc}</p></div></div>); })}</div></div>
      ...
      {showStrategyModal && strategyReport && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-fadeIn">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 max-w-sm w-full space-y-8 max-h-[85vh] overflow-y-auto shadow-2xl`}>
            <div className={`flex justify-between items-start border-b ${theme.border} pb-4`}><div><span className={`text-[10px] ${theme.muted} font-bold tracking-widest block uppercase mb-1`}>REPORTE IA</span><h3 className="text-3xl font-black leading-tight">Análisis.</h3></div><button onClick={() => setShowStrategyModal(false)} className={`${theme.muted} font-bold text-2xl transition-colors`}>✕</button></div>
            ...
            <div className={`space-y-6 border-t ${theme.border} pt-6`}>
              <div><span className={`text-[10px] ${theme.muted} font-bold uppercase tracking-widest block mb-3`}>Puntos Fuertes</span><ul className="space-y-2">{strategyReport.strengths.map((s, i) => <li key={i} className="text-sm font-medium flex gap-3"><span className={theme.muted}>•</span> {s}</li>)}</ul></div>
              <div><span className={`text-[10px] ${theme.muted} font-bold uppercase tracking-widest block mb-3`}>A Mejorar</span><ul className="space-y-2">{strategyReport.weaknesses.map((w, i) => <li key={i} className={`text-sm ${theme.muted} font-medium flex gap-3`}><span>•</span> {w}</li>)}</ul></div>
            </div>
            ...
            <button onClick={() => setShowStrategyModal(false)} className={`w-full py-4 ${theme.primary} font-black uppercase tracking-widest rounded-full text-[10px]`}>CERRAR REPORTE</button>
          </div>
        </div>
      )}
      ...
      {showHistoryModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6 animate-fadeIn">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-6 max-w-sm w-full space-y-5 shadow-2xl`}>
            <div className={`flex justify-between items-center border-b ${theme.border} pb-4`}><h3 className="text-lg font-black">Histórico</h3><button onClick={() => setShowHistoryModal(false)} className={`${theme.muted} font-bold text-xl`}>✕</button></div>
              ...
              {Array.from({ length: realDay }, (_, i) => i + 1).map((d) => {
                ...
                return (
                  <div key={d} className={`${theme.secondary} p-4 rounded-2xl border ${theme.border} flex justify-between items-center`}>
                    <div><span className="font-bold block text-sm">Día {d}</span><span className={`text-[10px] font-bold uppercase tracking-widest ${theme.muted}`}>{nut.cal} kcal | {nut.protein}g Prot</span></div>
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 max-w-sm w-full space-y-6 shadow-2xl`}>
            <div className={`flex justify-between items-center border-b ${theme.border} pb-4`}><h3 className="text-xl font-black">Ajustes</h3><button onClick={() => setShowSettingsModal(false)} className={`${theme.muted} font-bold text-xl`}>✕</button></div>
        ...
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-6 animate-fadeIn backdrop-blur-xl">
          <div className={`${theme.card} border ${theme.border} rounded-[2.5rem] p-8 max-w-sm w-full space-y-8 shadow-2xl relative overflow-hidden`}>
            ...
            <div className={`flex justify-between items-start pb-2 relative z-10`}>
              <div><span className={`text-[10px] ${theme.primary} px-3 py-1 rounded-full font-bold tracking-widest block uppercase mb-3 inline-block`}>STUDIO PRO</span><h3 className="text-3xl font-black leading-tight">Desbloquea<br/>tu potencial.</h3></div>
              <button onClick={() => setShowPaywallModal(false)} className={`${theme.muted} font-bold text-2xl transition-colors`}>✕</button>
            </div>
            <p className={`text-sm ${theme.muted} font-medium leading-relaxed relative z-10`}>Has agotado tus 10 tokens gratuitos de hoy. Únete a PRO para escanear comidas ilimitadas y generar recetas sin frenos.</p>
            <div className={`space-y-4 relative z-10`}>
              <div className="flex items-center gap-3 text-sm font-medium"><span className="text-green-500 text-lg">✓</span> Escaneo de IA ilimitado</div>
              <div className="flex items-center gap-3 text-sm font-medium"><span className="text-green-500 text-lg">✓</span> Recetas del Chef IA ilimitadas</div>
              <div className="flex items-center gap-3 text-sm font-medium"><span className="text-green-500 text-lg">✓</span> Análisis estratégico avanzado</div>
            </div>
            <div className="relative z-10 space-y-3">
              <div className="flex items-end justify-center gap-1 mb-6"><span className="text-4xl font-black">9.99€</span><span className={`text-xs ${theme.muted} font-bold uppercase tracking-widest pb-1`}>/ mes</span></div>
              <button onClick={() => { setIsPro(true); localStorage.setItem(`${userKey}_is_pro`, 'true'); setShowPaywallModal(false); alert('¡Pago simulado con éxito! Ahora eres usuario PRO.'); }} className={`w-full py-5 ${theme.primary} font-black uppercase tracking-widest rounded-[2rem] text-xs shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-105 transition-transform`}>Suscribirse Ahora</button>
              <button onClick={() => setShowPaywallModal(false)} className={`w-full py-4 bg-transparent ${theme.muted} font-bold uppercase tracking-widest rounded-full text-[10px]`}>Quizás más tarde</button>
            </div>
          </div>
        </div>
      )}
